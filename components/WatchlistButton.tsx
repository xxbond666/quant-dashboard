...
const WatchlistButton = ({
    ...
    const label = useMemo(() => {
        ...
        return added
            ? translate(locale, "watchlistBtn.remove")
            : translate(locale, "watchlistBtn.add");
    }, [added, type]);
    ...
    if (type === "icon") {
        return (
            <button
                ...
                className={`flex items-center justify-center p-2 rounded-full transition-all ${added ? "text-yellow-400 hover:bg-yellow-400/10" : "text-muted-foreground hover:text-foreground hover:bg-white/10"} ${loading ? "opacity-50 cursor-wait" : ""}`}
                onClick={handleClick}
                disabled={loading}
            >
                <svg
